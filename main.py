...
def single_file(prompt:str, model: str, fileName: str):
    ...
    # pipeline.load_textual_inversion("models/charturnerv2.pt", token="charturnerv2")

    print("model: {}".format(model))

    result = pipeline(prompt, num_inference_steps=20, guidance_scale=9).images

    print("Generate Finished: {}".format(fileName))

    for r in range(len(result)):
        result[r].save("results/"+fileName+str(r)+".png")
        print("File saveed to {}".format("results/"+fileName+str(r)+".png"))

def generate_video():
    pipe = DiffusionPipeline.from_pretrained("damo-vilab/text-to-video-ms-1.7b", torch_dtype=torch.float16, variant="fp16").to("mps")
...
def main( model: str, prompt: str, fileName: str,mode="image"):
    ...
    if mode == "single":
        single_file(prompt, model, fileName)
...
main("models/Realistic_Vision_V5.1.ckpt", "hyperrealistic portrait of a man as astronaut, portrait, well lit, cyberpunk,", "test", "single")